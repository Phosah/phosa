<script setup lang="ts">
import { defineProps } from 'vue';

const props = defineProps(['project']);
const project = props.project as any;
</script>
<template>
    <div
      class="group md:flex md:items-center md:space-x-12 my-12 py-10 md:py-20 px-4 
             border-2 border-transparent rounded-2xl 
             bg-[#151515] 
             transition-all duration-300 ease-in-out 
             hover:bg-[#1f1f1f] 
             hover:shadow-2xl hover:-translate-y-1"
    >
      <!-- Text Content -->
      <div class="md:flex-1 mb-10 md:mb-0 transition-colors duration-300 ease-in-out">
        <h6 
          class="mb-2 text-xl text-brand-gray font-semibold 
                 group-hover:text-yellow-600"
        >
          Role
        </h6>
  
        <h5 
          class="mb-2 text-xl font-bold text-yellow-700 
                 group-hover:text-yellow-500 transition-colors"
        >
          {{ project.subtitle }}
        </h5>
  
        <h3
          class="project-title mb-8 text-2xl md:text-3xl lg:text-4xl font-bold text-white 
                 tracking-wider leading-relaxed 
                 group-hover:text-gray-300"
        >
          {{ project.title }}
        </h3>
  
        <p
          class="mb-8 text-base md:text-lg text-brand-gray font-semibold tracking-wide 
                 group-hover:text-brand-gray-light"
        >
          {{ project.description }}
        </p>
  
        <div class="flex items-center justify-between">
          <!-- View Site Link -->
          <div class="flex items-center space-x-2 text-white">
            <p class="group-hover:text-yellow-500 transition-colors">View Site</p>
            <a :href="project.livelink" target="_blank"
               class="transform transition-transform duration-300 group-hover:translate-x-2"
            >
              <svg
                class="w-6 h-6 transition-colors duration-300 group-hover:text-yellow-500"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
                  fill-rule="evenodd"
                  d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z"
                  clip-rule="evenodd"
                />
              </svg>
            </a>
          </div>
  
          <!-- GitHub Icon -->
          <div
            class="w-10 h-10 flex items-center justify-center 
                   bg-[#202020] rounded-full 
                   transition-all duration-300 ease-in-out
                   group-hover:bg-yellow-600 transform group-hover:scale-105"
          >
            <a :href="project.githubLink" target="_blank">
              <img src="../assets/github.png" alt="Github" class="w-6 h-6" />
            </a>
          </div>
        </div>
      </div>
  
      <!-- Image -->
      <div class="md:flex-1 max-h-72 rounded-md overflow-hidden">
        <img
          class="max-h-72 w-full object-cover rounded-xl 
                 transition-transform duration-500 ease-in-out
                 group-hover:scale-105"
          :src="project.img"
          :alt="project.alt"
        />
      </div>
    </div>
  </template>
  

<!-- <template>
    <div class="md:flex md:items-center md:space-x-12 my-12 py-10 md:py-20 px-4 border-2 rounded-2xl border-transparent hover:border-yellow-700 transition-all">
        <div class="md:flex-1 mb-10 md:mb-0">
            <h6 class="mb-2 text-xl text-brand-gray font-semibold">Role</h6>
            <h5 class="mb-2 text-xl font-bold text-yellow-700">{{ project.subtitle }}</h5>
            <h3
                class="mb-8 text-2xl md:text-3xl lg:text-4xl font-bold text-white tracking-wider leading-relaxed"
            >{{ project.title }}</h3>
            <p
                class="mb-8 text-base md:text-lg text-brand-gray font-semibold tracking-wide"
            >{{ project.description }}</p>
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2 text-white">
                    <p>View Site</p>
                    <a :href="project.livelink" target="_blank">
                        <svg
                            class="w-6 h-6 hover:transition-all hover:translate-x-1 hover:text-yellow-600"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                fill-rule="evenodd"
                                d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z"
                                clip-rule="evenodd"
                            />
                        </svg>
                    </a>
                </div>
                <div class="w-10 h-10 flex items-center justify-center bg-[#202020] rounded-full hover:bg-yellow-600">
                    <a :href="project.githubLink">
                        <img class src="../assets/github.png" alt="Github" />
                    </a>
                </div>
            </div>
        </div>

        <div class="md:flex-1 max-h-72 rounded-md">
            <img class="max-h-72 w-full object-cover my-auto  rounded-xl" :src="project.img" :alt="project.alt" />
        </div>
    </div>
</template> -->

<style>
    .project-title {
        font-family: "NeueMachina-Ultrabold.otf";
    }
</style>